...
import mimetype from '../epub_templates/dict/mimetype'
import container from '../epub_templates/dict/META-INF/container.xml'
import opf from '../epub_templates/dict/EPUB/dict.opf'
import ncx from '../epub_templates/dict/EPUB/dict.ncx'
import nav from '../epub_templates/dict/EPUB/nav.html'
import css from '../epub_templates/dict/EPUB/css/main.css'
import content from '../epub_templates/dict/EPUB/content.html'
import autoToc from '../epub_templates/dict/EPUB/auto-toc.html'
import sectionsNavTemplate from '../epub_templates/dict/EPUB/sections-nav-template.html'
import sectionsNCXTemplate from '../epub_templates/dict/EPUB/sections-ncx-template.xml'
import sectionsOPFManifestTemplate from '../epub_templates/dict/EPUB/sections-opf-manifest-template.xml'
import sectionsOPFSpineTemplate from '../epub_templates/dict/EPUB/sections-opf-spine-template.xml'
...
class Builder {
  ...
  addManifestOpf(zip, epubConfig) {
    ...
    zip.folder('EPUB').file('dict.opf', this.compile(templates.opf, epubConfig));
  }
  ...
  addEpub2Nav(zip, epubConfig) {
    ...
    zip.folder('EPUB').file('dict.ncx', this.compile(templates.ncx, epubConfig));
  }
  ...
  addSection(zip, section) {
    ...
    if (section.subSections && section.subSections.length) {
      zip.folder('EPUB').file(section.name + '_content.html', this.compile(templates.content, section));
    }
  }
